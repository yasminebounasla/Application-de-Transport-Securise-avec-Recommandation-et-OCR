...
export const submitFeedback = async (req, res) => {
  ...
  try {
    ...
    if (!trajet) return res.status(404).json({ message: "Trajet not found." });
...
export const getDriverFeedback = async (req, res) => {
  ...
  if (!driverId) return res.status(403).json({ message: "Access restricted to drivers only." });
  ...
  try {
    ...
    return res.status(200).json({
      message: "Feedback retrieved successfully.", data: feedbacks,
      pagination: {
        currentPage: page, totalPages: Math.ceil(totalFeedbacks / limit),
        totalFeedbacks, limit,
...
export const getPublicDriverStats = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({
      message: "Driver stats retrieved successfully.",
      data: {
        averageRating: driver.avgRating || 0, totalFeedbacks: driver.ratingsCount || 0,
...
export const getPublicDriverFeedback = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({
      message: "Public feedback retrieved successfully.", data: feedbacks,
      pagination: {
        currentPage: page, totalPages: Math.ceil(totalFeedbacks / limit),
        totalFeedbacks, limit,
        hasNextPage: page < Math.ceil(totalFeedbacks / limit),
