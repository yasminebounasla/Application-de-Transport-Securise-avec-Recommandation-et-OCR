...
function Flash({
  ...
  return (
    <View style={[s.flash, isSuccess ? s.flashSuccess : s.flashError]}>
      ...
      />
      <Text style={[s.flashText, { color: isSuccess ? "#16A34A" : "#DC2626" }]}>
        {flash.text}
      </Text>
    </View>
  );
...
function RideCard({
  ...
  const wasRejectedByMe = item.rejectedDriverIds?.some(
    (id: any) => Number(id) === Number(user?.driverId)
  );
  console.log('rejectedDriverIds:', item.rejectedDriverIds);
  console.log('driverId:', user?.driverId);
  console.log('wasRejectedByMe:', wasRejectedByMe);
  const cfg = wasRejectedByMe
    ? {
        label: "Cancelled by you",
        bg: "#FEE2E2",
        color: "#991B1B",
  ...
  const dateLabel = item.dateDepart
    ? new Date(item.dateDepart).toLocaleDateString("en-GB", {
        day: "2-digit",
        month: "short",
        year: "numeric",
      })
    ...
    >
          ...
          {hasPass ? (
            <>
              ...
              <View>
                ...
                <Text style={s.passengerSub}>
                  {formatPhoneNumberForDisplay(passenger.numTel)}
                </Text>
              </View>
            </>
          ) : (
        ...
        <View style={s.priceRow}>
          <View style={s.priceBadge}>
            ...
            <Text style={s.priceText}>
              {Number(item.prix).toLocaleString("fr-FR")} DA
            </Text>
          </View>
        </View>
      )}
      ...
      <View style={s.tagsRow}>
        ...
        {item.placesDispo != null && (
          <Tag
            icon="people-outline"
            label={`${item.placesDispo} seat${item.placesDispo > 1 ? "s" : ""}`}
          />
        )}
      </View>

      {/* ── PENDING ACTIONS ou footer ── */}
      {isPending && !wasRejectedByMe && onAccept && onReject ? (
          ...
          {SORT_OPTIONS.map((opt) => (
            ...
            >
              <Ionicons
                ...
                size={16}
                color={current === opt.key ? "#111" : "#9CA3AF"}
              />
              <Text
                style={[
                  s.modalRowText,
                  current === opt.key && s.modalRowTextActive,
                ]}
              >
                {opt.label}
              </Text>
  ...
  const categorized = useMemo(
    () => ({
      ...
      pending: activity.filter(
        (r: any) =>
          r.status === "PENDING" &&
          !r.rejectedDriverIds?.includes(user?.driverId),
      ),
      accepted: activity.filter((r: any) =>
        ["ACCEPTED", "IN_PROGRESS"].includes(r.status),
      ),
      cancelled: activity.filter(
        (r: any) =>
          ["CANCELLED_BY_PASSENGER", "CANCELLED_BY_DRIVER"].includes(
            r.status,
          ) ||
          (r.status === "PENDING" &&
            r.rejectedDriverIds?.includes(user?.driverId)),
  ...
  const visibleRides = useMemo(() => {
    ...
    if (nameQuery.trim()) {
      ...
      rides = rides.filter((r: any) =>
        `${r.passenger?.prenom || ""} ${r.passenger?.nom || ""}`
          .toLowerCase()
          .includes(q),
      );
    }
    ...
    if (activeCategory === "accepted") {
      rides.sort((a, b) => {
        ...
        if (a.status !== "IN_PROGRESS" && b.status === "IN_PROGRESS") return 1;
        return (
          new Date(a.dateDepart || 0).getTime() -
          new Date(b.dateDepart || 0).getTime()
        );
      });
      return rides;
    }
    ...
    if (sortKey === "price_desc")
      rides.sort((a, b) => (Number(b.prix) || 0) - (Number(a.prix) || 0));
    if (sortKey === "price_asc")
      rides.sort((a, b) => (Number(a.prix) || 0) - (Number(b.prix) || 0));
    if (sortKey === "name_az")
      rides.sort((a, b) =>
        (a.passenger?.prenom || "").localeCompare(b.passenger?.prenom || ""),
      );
    if (sortKey === "name_za")
      rides.sort((a, b) =>
        (b.passenger?.prenom || "").localeCompare(a.passenger?.prenom || ""),
      );
    return rides;
  ...
  const handleRideAction = async (
    ...
    try {
      ...
      if (action === "accept") {
        ...
        await loadActivity();
        setActiveCategory('cancelled')
      }
      await loadActivity();
    } catch (e: any) {
      Alert.alert(
        "Error",
        e?.response?.data?.message || `Failed to ${action} ride.`,
      );
    } finally {
        ...
        renderItem={({ item }) => {
          ...
          return (
            <RideCard
              ...
              onAccept={
                item.status === "PENDING"
                  ? () => confirmAction(item.rideId, "accept")
                  : undefined
              }
              onReject={
                item.status === "PENDING"
                  ? () => confirmAction(item.rideId, "reject")
                  : undefined
              }
