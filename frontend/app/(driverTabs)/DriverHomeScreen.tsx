...
function buildWeekPeriods(rides: DriverActivityItem[], accountCreatedAt?: string | null): WeekPeriod[] {
  ...
  const accountStart = accountCreatedAt ? startOfDay(new Date(accountCreatedAt)) : today;
  ...
  return Array.from({ length: totalWeeks }, (_, index) => {
    ...
    return {
      key: `week-${index}`,
      title: index === 0 ? 'Last 7 days' : index === 1 ? '1 week ago' : `${index} weeks ago`,
...
function buildMonthPeriods(rides: DriverActivityItem[], accountCreatedAt?: string | null): MonthPeriod[] {
  const today = startOfDay(new Date());
  const accountStart = accountCreatedAt ? startOfDay(new Date(accountCreatedAt)) : today;
  const currentMonthStart = new Date(today.getFullYear(), today.getMonth(), 1);
  const accountMonthStart = new Date(accountStart.getFullYear(), accountStart.getMonth(), 1);
  ...
  return Array.from({ length: totalMonths }, (_, index) => {
    const monthStart = addMonths(new Date(today.getFullYear(), today.getMonth(), 1), -index);
    ...
    const realDays = points.filter((point) => !point.isPlaceholder && !point.isBeforeAccount);
    ...
    return {
      key: `month-${index}`,
      title: index === 0 ? 'This month' : index === 1 ? '1 month ago' : `${index} months ago`,
      subtitle: monthStart.toLocaleDateString('en-US', { month: 'long', year: 'numeric' }),
      total,
      completed,
      bestLabel: hasProgress && bestPoint.dayNumber ? `Day ${bestPoint.dayNumber}` : '-',
...
function WeeklyBar({
  ...
  const targetHeight = maxValue > 0 ? Math.max(16, (value / maxValue) * 112) : 16;
  ...
  return (
    <View style={styles.weekBarCol}>
      ...
      <View style={styles.weekBarTrack}>
        <Animated.View style={[styles.weekBarFill, { height: animatedHeight, backgroundColor: color }]} />
      </View>
...
function ActionCard({
  ...
  return (
    <TouchableOpacity
      activeOpacity={0.86}
      onPress={onPress}
      style={[styles.actionCard, dark && styles.actionCardDark, muted && styles.actionCardMuted]}
    >
      <View style={[styles.actionIconWrap, dark && styles.actionIconWrapDark, muted && styles.actionIconWrapMuted]}>
        <MaterialIcons name={icon} size={20} color={dark ? '#fff' : muted ? '#8A6516' : '#111'} />
      </View>
      <View style={styles.actionTextWrap}>
        <Text style={[styles.actionTitle, dark && styles.actionTitleDark, muted && styles.actionTitleMuted]}>{title}</Text>
        <Text style={[styles.actionSubtitle, dark && styles.actionSubtitleDark, muted && styles.actionSubtitleMuted]}>{subtitle}</Text>
      </View>
      <MaterialIcons name="arrow-forward-ios" size={16} color={dark ? '#fff' : muted ? '#8A6516' : '#111'} />
    </TouchableOpacity>
  );
}
...
export default function HomeScreen() {
  ...
  const [accountCreatedAt, setAccountCreatedAt] = useState<string | null>((user as any)?.createdAt || null);
  ...
  const loadStats = useCallback(async () => {
    ...
    try {
      ...
      const completed = driverRides.filter((ride: DriverActivityItem) => ride.status === 'COMPLETED').length;
      const accepted = driverRides.filter((ride: DriverActivityItem) => ['ACCEPTED', 'IN_PROGRESS'].includes(ride.status)).length;
      const cancelled = driverRides.filter((ride: DriverActivityItem) => ['CANCELLED_BY_PASSENGER', 'CANCELLED_BY_DRIVER'].includes(ride.status)).length;
      const total = driverRides.length;
      const completionRate = total > 0 ? Math.round((completed / total) * 100) : 0;

      setRides(driverRides);
      setActivityStats({ total, completed, accepted, cancelled, completionRate });
    } catch (error) {
      if (error?.response?.status === 401) {
        await logout();
        setRides([]);
        setActivityStats({ total: 0, completed: 0, accepted: 0, cancelled: 0, completionRate: 0 });
        return;
      }
  ...
  useEffect(() => {
    Animated.loop(
      Animated.sequence([
        Animated.timing(statusPulse, { toValue: 1.18, duration: 700, useNativeDriver: true }),
        Animated.timing(statusPulse, { toValue: 1, duration: 700, useNativeDriver: true }),
      ])
    ).start();
  }, [statusPulse]);

  const weekPeriods = useMemo(() => buildWeekPeriods(rides, accountCreatedAt), [accountCreatedAt, rides]);
  const monthPeriods = useMemo(() => buildMonthPeriods(rides, accountCreatedAt), [accountCreatedAt, rides]);
  const activePeriods = periodMode === 'weeks' ? weekPeriods : monthPeriods;
  const displayPeriods = useMemo(() => [...activePeriods].reverse(), [activePeriods]);
  const latestIndex = Math.max(displayPeriods.length - 1, 0);
  const currentPeriod = displayPeriods[displayIndex] || displayPeriods[latestIndex];
  ...
  const animatePeriodInfo = useCallback((nextIndex: number) => {
  ...
  const switchMode = useCallback((mode: 'weeks' | 'months') => {
  ...
  }, [getDriverActiveRide, loadAccountCreatedAt, loadDashboardSummary, loadStats]);
    ...
    >
      ...
      <Animated.View
        style={[
          ...
          { opacity: dashboardFade, transform: [{ translateY: dashboardLift }] },
        ]}
      >
        <View style={styles.sectionHead}>
          <Animated.View style={{ flex: 1, opacity: periodContentFade }}>
            <Text style={styles.sectionTitle}>{currentPeriod?.title || 'Recent activity'}</Text>
            <Text style={styles.sectionSubtitle}>{currentPeriod?.subtitle || 'Browse your driver activity'}</Text>
          </Animated.View>
          <View style={styles.modeSwitch}>
            <TouchableOpacity
              activeOpacity={0.9}
              style={[styles.modeChip, periodMode === 'weeks' && styles.modeChipActive]}
              onPress={() => switchMode('weeks')}
            >
              <Text style={[styles.modeChipText, periodMode === 'weeks' && styles.modeChipTextActive]}>
                Weeks
              </Text>
            </TouchableOpacity>
            <TouchableOpacity
              activeOpacity={0.9}
              style={[styles.modeChip, periodMode === 'months' && styles.modeChipActive]}
              onPress={() => switchMode('months')}
            >
              <Text style={[styles.modeChipText, periodMode === 'months' && styles.modeChipTextActive]}>
                Months
              </Text>
            </TouchableOpacity>
          </View>
        ...
        <Text style={styles.swipeHint}>
          Swipe from left to right to go to older {periodMode === 'weeks' ? 'weeks' : 'months'}.
        </Text>
        ...
        <Animated.View style={{ opacity: periodContentFade }}>
          <View style={styles.weekSummaryRow}>
            <View style={styles.weekSummaryCard}>
              ...
              <Text style={styles.weekSummaryValue}>{currentPeriod?.bestLabel || '-'}</Text>
              <Text style={styles.weekSummaryMeta}>
                {currentPeriod?.hasProgress ? `${currentPeriod?.bestValue || 0} trips handled` : 'No progress yet'}
              </Text>
            </View>
            <View style={[styles.weekSummaryCard, styles.weekSummaryCardWarm]}>
              ...
              <Text style={styles.weekSummaryValue}>{currentPeriod?.total || 0}</Text>
              ...
              {periodMode === 'weeks' ? (
                <View style={styles.chartWrap}>
                  {(item as WeekPeriod).points.map((point) => {
                    const maxValue = Math.max(...(item as WeekPeriod).points.map((entry) => entry.total), 1);
                    return (
                      <WeeklyBar
                        key={point.key}
                        label={point.label}
                        value={point.total}
                        maxValue={maxValue}
                        color={point.total === 0 ? '#D4D4D4' : point.completed > 0 ? '#16A34A' : '#F59E0B'}
                      />
                    );
                  })}
                </View>
                ...
                <View style={styles.calendarWrap}>
                  ...
                  <Text style={styles.calendarMonthLabel}>{(item as MonthPeriod).monthLabel}</Text>
                </View>
              )}
            </View>
          )}
        ...
        <Animated.View style={{ opacity: periodContentFade }}>
          ...
          >
            ...
            <View style={[styles.progressTrack, { backgroundColor: progressTheme.track }]}>
              <View
                style={[
                  styles.progressFill,
                  {
            ...
            <View style={styles.progressLegend}>
              <View style={styles.progressLegendRow}>
                <View style={[styles.progressLegendDot, { backgroundColor: progressTheme.fill }]} />
                <Text style={[styles.progressLegendText, { color: progressTheme.hint }]}>
                  {progressTheme.text}
                </Text>
              </View>
            </View>
      ...
      <View style={styles.earningsCard}>
        <View style={styles.earningsHeader}>
          <View>
            ...
            <Text style={styles.earningsSubtitle}>From all completed trips</Text>
          </View>
      ...
      <View style={styles.quickSection}>
        ...
        <ActionCard
          ...
          onPress={() =>
            inProgressRide ? openInProgress() : router.push('/(driverTabs)/Activity' as any)
          }
        />
      </View>
    </ScrollView>
