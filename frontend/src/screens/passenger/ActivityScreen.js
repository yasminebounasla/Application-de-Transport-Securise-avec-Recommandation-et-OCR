...
export default function ActivityScreen() {
  ...
  const categorized = useMemo(() => {
    const completed = activity.filter((ride) => ride.status === 'COMPLETED');
    const pending = activity.filter((ride) =>
      ['PENDING', 'ACCEPTED', 'IN_PROGRESS'].includes(ride.status)
    );
    const cancelled = activity.filter((ride) =>
  ...
  const stats = useMemo(() => {
    ...
    const completed = activity.filter((ride) => ride.status === 'COMPLETED').length;
    const pending = activity.filter((ride) =>
      ['PENDING', 'ACCEPTED', 'IN_PROGRESS'].includes(ride.status)
    ).length;
    const cancelled = activity.filter((ride) =>
  ...
  const visibleRides = useMemo(() => {
    let rides = categorized[activeCategory] || [];
  ...
  const cycleFilter = (current, values) => {
