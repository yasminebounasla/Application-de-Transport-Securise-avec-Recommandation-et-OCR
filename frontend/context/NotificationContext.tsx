...
const CATEGORY_TOAST = (title: string): { color: string; icon: any } => {
  ...
  if (t.includes("confirme") || t.includes("accepte"))
    return { color: "#22C55E", icon: "checkmark-circle" };
  if (t.includes("refus") || t.includes("annule") || t.includes("expiree"))
    return { color: "#EF4444", icon: "close-circle" };
  if (t.includes("envoyee") || t.includes("cree") || t.includes("demande"))
...
export const NotificationProvider = ({ children }: { children: ReactNode }) => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (cached) {
          ...
          setNotifications(
            parsed.map((n, i) => ({ ...n, isRead: i < count ? false : true })),
          );
        }
  ...
  const fetchFromDB = useCallback(
    async (retryCount = 0) => {
      ...
      try {
        ...
        if (!token) {
          if (retryCount < 3)
            setTimeout(() => fetchFromDB(retryCount + 1), 1000);
          return;
        }
        ...
        if (response.ok) {
          ...
          const normalized: Notification[] = data.map((n: any, i: number) => ({
            ...
            photoUrl:
              n.data?.passenger?.photoUrl ||
              n.data?.driver?.photoUrl ||
              n.data?.photoUrl,
        ...
        console.error("fetchFromDB error:", e);
      }
    },
  ...
  const addNotif = useCallback(
    async (title: string, message: string, extra?: Partial<Notification>) => {
  ...
  const removeNotifsByRideId = useCallback(
    (rideId: number) => {
      setNotifications((prev) => {
